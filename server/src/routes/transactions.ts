...
import { createTransactionSchema, verifyTransactionSchema, submitToSwiftSchema } from "../utils/transactionValidators";
...
router.get("/pending", transactionLimiter, async (req, res) => {
  try {
    ...
    const transactions = await Transaction.find({ status: "pending" })
      ...
      .select("amount currency payeeAccountInfo swiftCode createdAt")
      .sort({ createdAt: -1 })
      .lean();
